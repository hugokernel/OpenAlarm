  ...
  if ('0' <= c && c <= '9')
    ...
    switch (c) {
      ...
      case 'x': // set reporting mode to hex (1) or decimal (0)
        ...
        case 'h':
           send_no_sleep = true; 
           Serial.println("Next ack send no sleep command !");
           break;
    }
    value = top = 0;
  ...
  if (rf12_recvDone()) {
    ...
    if (rf12_crc == 0) {
      ...
      if (RF12_WANTS_ACK && (config.nodeId & COLLECT) == 0) {
        ...
        if (send_no_sleep) {
            Serial.println(" -> ack (special)");
            uint8_t datA[1];
            datA[0] = 8;
            rf12_sendStart(RF12_ACK_REPLY, datA, 1);
            send_no_sleep = false;
        } else {
            Serial.println(" -> ack");
